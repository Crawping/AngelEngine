...
namespace AngelGUISystem
{
	namespace GUI3D
	{
		...
		public:
			...
			void SetFontStyle(DWRITE_FONT_STYLE style) { this->m_font->style = style; }
			void SetFontLocal(const wchar_t * _local) { this->m_font->local = _local; }
			...
			void SetCaretColor(float &r, float &g, float &b, float &a);
			///
		...
		private:
			...
			Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>
				LoadOptionIcon(std::wstring name);
		...
		private:
			...
			D2D1::Matrix3x2F	m_matrix2DTransform;
			AGSprite *			m_scrollBar;
			//---------- Mouse intersection flags
			bool		m_intersectsCopyOption;
			bool		m_intersectsPasteOption;
			bool		m_intersectsCutOption;
			bool		m_intersectsfontOption;
			bool		m_showOptions;
			bool		m_updateOptionsTransform;
			//--------------------------------

			//---------- Textures
			Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>		m_copyTexture;
			Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>		m_pasteTexture;
			Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>		m_cutTexture;
			Microsoft::WRL::ComPtr<ID3D11ShaderResourceView>		m_fontTexture;
			//--------------------

			GUI3DRendering::CBuffer<World>								m_cbWorldCopy;
			GUI3DRendering::CBuffer<World>								m_cbWorldPaste;
			GUI3DRendering::CBuffer<World>								m_cbWorldCut;
			GUI3DRendering::CBuffer<World>								m_cbWorldfont;
			GUI3DRendering::CBuffer<ColorBuffer>						m_cbColor;
			GUI3DRendering::CBuffer<ColorBuffer>						m_cbColorOptionsCopy;
			GUI3DRendering::CBuffer<ColorBuffer>						m_cbColorOptionsPaste;
			GUI3DRendering::CBuffer<ColorBuffer>						m_cbColorOptionsCut;
			GUI3DRendering::CBuffer<ColorBuffer>						m_cbColorOptionsFont;
			DirectX::XMFLOAT3											m_pointInLocalSpace;

			//----------- 3D space transformation
			DirectX::XMFLOAT4X4 m_copyOptionWorld;
			DirectX::XMFLOAT4X4 m_pasteOptionWorld;
			DirectX::XMFLOAT4X4 m_cutOptionWorld;
			DirectX::XMFLOAT4X4 m_fontOptionWorld;
			//--------------------

			//----------- 3D space positions
			DirectX::XMFLOAT3 m_copyOptionPosition;
			DirectX::XMFLOAT3 m_pasteOptionPosition;
			DirectX::XMFLOAT3 m_cutOptionPosition;
			DirectX::XMFLOAT3 m_fontOptionPosition;
			//--------------------

			//store mouse position for later use;
			DirectX::XMFLOAT2 m_mousePositionScreen;
			bool m_isMouseSelectedOptions;
		};
	}
}
